...
rob = readtable('tscan_UR_051219_2.txt');
for i = 1:size(rob,1)
...
datLT = readtable('tscan_lt_051219_2.txt');
data.LT = [datLT{:,1},datLT{:,2:4}*1.e-03];
...
% MV = [16 -5 369]*1e-03; % check Sign tool0 zu CCR (das es von mm in Meter ist)
...
if var.leverarmRF    
    MV = [0 -0 370]*1e-03; % HIER NÄHERUNGSWERTE EINGEBEN IN MM (tool0 zu CCR)
    for i = 1:size(data.R,1)
        rot = ltRot(data.R(i,5:7));
        pt(i,:) = [rot * MV' + data.R(i,2:4)']';
    end
    [tp,rc,ac,tr] = helmert3d(pt(:,:),data.LT(:,2:4),'7p',1,[0,0,0]);
...
if var.cov == 1
    ...
    adj.Cll_R = eye(3,3)*.18e-03^2; % Kov ^2 %HIER A PRIO ANNAHME ÄNDERN GT
